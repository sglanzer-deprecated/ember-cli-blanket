...
/**
 * HTMLReporter outputs HTML-formatted coverage data
 * from the test run. Heavily borrows from blanket's own default qunit reporter.
 *
 * @class  HTMLReporter
 * @param {Object} coverage information received from blanket
 */
module.exports = Reporter.extend({
  ...
  transform: function(coverage) {
    var cssStyle = "#blanket-main {margin:2px;background:#EEE;color:#333;clear:both;font-family:'Helvetica Neue Light', 'HelveticaNeue-Light', 'Helvetica Neue', Calibri, Helvetica, Arial, sans-serif; font-size:17px;} #blanket-main a {color:#333;text-decoration:none;}  #blanket-main a:hover {text-decoration:underline;} .blanket {margin:0;padding:5px;clear:both;border-bottom: 1px solid #FFFFFF;} .bl-error {color:red;}.bl-success {color:#5E7D00;} .bl-file{width:auto;} .bl-cl{float:left;} .blanket div.rs {margin-left:50px; width:150px; float:right} .bl-nb {padding-right:10px;} #blanket-main a.bl-logo {color: #EB1764;cursor: pointer;font-weight: bold;text-decoration: none} .bl-source{ overflow-x:scroll; background-color: #FFFFFF; border: 1px solid #CBCBCB; color: #363636; margin: 25px 20px; width: 80%;} .bl-source div{white-space: pre;font-family: monospace;} .bl-source > div > span:first-child{background-color: #EAEAEA;color: #949494;display: inline-block;padding: 0 10px;text-align: center;width: 30px;} .bl-source .hit{background-color:#c3e6c7} .bl-source .miss{background-color:#e6c3c7} .bl-source span.branchWarning{color:#000;background-color:yellow;} .bl-source span.branchOkay{color:#000;background-color:transparent;}",
      successRate = 60,
      fileNumber = 0,
      hasBranchTracking = this.options.branchTracking === true,
    ...
    var appendTag = function (type, str) {
      html += "<" + type + ">";
      html += str;
      html += "</" + type + ">";
    };
    ...
    var isNotEmpty = function(item) { return typeof item !== 'undefined' && item !== null; };

    var files = coverage.fileData;
    var totals = {
      totalSmts: 0,
    ...
    for(var file in files)
    {
      ...
      var statsForFile = files[file],
        ...
        code = [],
        i,
        branchData = statsForFile.branchData;
      ...
      for(i = 0; i < statsForFile.source.length; i +=1){
        ...
        if (branchStack.length > 0 ||
          (isNotEmpty(branchData) && branchData.length > 0))
        {
          if (isNotEmpty(branchData[i+1]))
          {
            var cols = branchData[i+1].filter(isNotEmpty);
            var colsIndex=0;
        ...
        var lineClass="";
        if(statsForFile.lines[i+1]) {
          numberOfFilesCovered += 1;
          totalSmts += 1;
          lineClass = 'hit';
        }else{
          if(statsForFile.lines[i+1] === 0){
            totalSmts++;
            lineClass = 'miss';
          }
        }
      ...
      if (isNotEmpty(branchData)){
        for(var j=0;j<branchData.length;j++){
          if (isNotEmpty(branchData[j])){
            for(var k=0;k<branchData[j].length;k++){
              if (isNotEmpty(branchData[j][k])){
                totalBranches++;
                if (isNotEmpty(branchData[j][k][0]) &&
                  branchData[j][k][0].length > 0 &&
                  isNotEmpty(branchData[j][k][1]) &&
                  branchData[j][k][1].length > 0){
                  passedBranches++;
                }
              }
            }
      ...
      if (modulePatternRegex) {
        var moduleName = statsForFile.fileName.match(modulePatternRegex)[1];
      ...
      var output = fileTemplate.replace("{{file}}", statsForFile.fileName)
    ...
    html += "<head>";
    appendTag('style', cssStyle);
    html += "</head>";
    html += "<body>";

    html += '<script type="text/javascript"/>';
    html += "function blanket_toggleSource(id) {" +
